...
LocalFile.prototype.init = function (callback) {
  ...
  this.fetch(function (err, localDirectory) {
    ...
    winston.debug('Copying app to ' + self.appDir.bold);
    var app = self.app.user + '/' + self.app.name;

    exec('cp -r ' + localDirectory + ' ' + self.appDir, function (err) {
      self.stat(function (err, exists) {
        if (exists) {
          winston.warn('The app ' + app.bold + ' exists already');
        }
        ...
        winston.debug('Trying to install dependencies for ' + self.appDir.bold);
