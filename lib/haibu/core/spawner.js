...
haibu.getSpawnOptions = function getSpawnOptions (app) {
  ...
  var env, key, value, _ref, _ref1;

  env = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../../../../../server/env.json')));

  if (env[app.user] && env[app.user][app.name]) {
    winston.warn('Attempting to inject env vars for ' + ("" + app.user + "/" + app.name).bold);
    if ((_ref = options.env) == null) {
      options.env = {};
    }
    _ref1 = env[app.user][app.name];
    for (key in _ref1) {
      value = _ref1[key];
...
Spawner.prototype.trySpawn = function (app, callback) {
  var self = this,
      repo,
      appName = app.user + '/' + app.name;

  // Prefix the default init of repo for spawn.
  winston.warn('Attempting to cleanup ' + appName.bold + ' if exists');

  // Before attempting any spawn, remove the local repo.
  var dir = path.resolve(__dirname, '../../../local/' + app.user + '/' + app.name);
  ...
  repo.bootstrap(function (err, existed) {
    ...
    repo.init(function (err) {
      if (err) {
        ...
        winston.debug('Attempting to spawn ' + appName.bold);
...
Spawner.prototype.spawn = function spawn (repo, callback) {
  ...
  var log = path.resolve(__dirname, '../../../../../server/logs/' + app.name);
  ...
  foreverOptions = {
    ...
    outFile:   log + '.out', // Path to log output from child stdout
    errFile:   log + '.err'  // Path to log output from child stderr
  };
